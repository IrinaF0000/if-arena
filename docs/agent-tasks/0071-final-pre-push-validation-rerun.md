# Task 0071: Final pre-push validation rerun

## Goal

Rerun and record the final local validation gate for the playable pre-push milestone after fixing the TCP vertical smoke blocker from task 0070.

## Scope

Allowed files:

- `docs/agent-runs/0071-final-pre-push-validation-rerun.md`
- `docs/agent-tasks/0071-final-pre-push-validation-rerun.md`
...
- Final run note records branch, HEAD, untracked/generated files, validation commands, results, and residual risks.
- C++ build/test, frontend checks, protocol/frontend tests, gameplay scenario tests, CI validators, agent validators, security scan, local smoke tests, and Browser smoke are run when available.
...
- Browser smoke of the built Mini App.
- `git diff --check`
...
Revert this task commit to remove the final validation rerun record only.
