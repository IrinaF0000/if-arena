...
- `tick` is the deterministic driver for tests and future workers: it drains a bounded command queue, applies commands to `BattleEngine`, and queues readable event/snapshot payloads for each matched session.
- The default Objective Run match uses the canonical bottom-to-top arena, direct spawn-to-objective lanes, configured hazards, automatic objective pickup/capture, and a smooth per-tick player speed tuned for the live server tick rate.
