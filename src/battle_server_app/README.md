...
`battle_server_app` now loads and validates JSON config, initializes backend resource limits, and starts the local raw TCP listener when `transports.tcp.enabled` is true. WebSocket config is still parsed and logged, but the WebSocket listener is skipped until task 0026.
